...
//   try {
//     const imageUrl = screenshot?.screenshotUrl || screenshot?.url;
    
//     if (!imageUrl) {
//       console.warn('No valid URL found in screenshot:', screenshot);
//       return null;
//     }
...
//                 ...screenshot,
//                 url: base64Url,
//                 timestamp: screenshot.timestamp || screenshot.time || new Date().toISOString(),
...
async function getImageAsBase64(screenshot) {
  ...
  async function generatePDF(reportData, reportId) {
    ...
    try {
      ...
      if (reportData.evidence?.screenshots?.length) {
        console.log('Processing screenshots:', reportData.evidence.screenshots.length);
        
        const screenshotsWithBase64 = await Promise.all(
          reportData.evidence.screenshots
            .filter(screenshot => screenshot && (screenshot.screenshotUrl || screenshot.url))
            .map(async screenshot => {
                try {
                  const base64Url = await getImageAsBase64(screenshot);
                  if (!base64Url) {
...
function generateHTML(reportData, reportId) {
  const template = `
    ...
    <body>
      <div class="header">
        <img src="{{logoUrl}}" alt="SafeDM Logo" class="logo">
    ...
    {{/if}}
      ...
      <div class="section">
        ...
        <p>This report was automatically generated by SafeDM's Digital Safety Platform on {{generatedDate}}.</p>
        ...
        <p>While SafeDM employs industry-standard practices to ensure data accuracy and integrity, this report should be reviewed by qualified legal professionals before use in any legal proceedings. SafeDM provides no warranty regarding the completeness or accuracy of the information contained herein.</p>
