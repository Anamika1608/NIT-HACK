  ...
  try {
    ...
    if (!name || !profileUrl) {
      return res.status(400).json({
        status: 'error',
        type: 'ValidationError',
        message: 'Name and profileUrl are required'
      });
    }
...
router.get('/hidden-messages', authMiddleware, async (req, res) => {
  try {
      ...
      if (userName) {
          pipeline.push({
              $match: { userName: { $regex: new RegExp(userName.toLowerCase(), 'i') } }
          });
      }

      if (platform) {
          pipeline.push({ $match: { platform: platform.toLowerCase() } });
      }
      ...
      res.status(200).json({
          ...
          data: {
              ...
              pagination: {
                  currentPage: pageNum,
                  totalPages: Math.ceil((totalCount[0]?.total || 0) / limitNum),
